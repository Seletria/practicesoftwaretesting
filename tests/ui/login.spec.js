...
test.describe('Login Tests', () => {
  ...
  test('should redirect user to account after login @smoke', async ({ page }) => {
  ...
  test('should display error message when email or password is wrong @regression', async ({ page }) => {
  ...
  test('should display error message when email is empty @regression', async ({ page }) => {
    await loginPage.login('', process.env.CUSTOMER_PASSWORD);

    await loginPage.expectEmptyEmailError('Email is required');
  })

  test('should display error message when password is empty @regression', async ({ page }) => {
